...
export class UserManager extends OidcClient {
    ...
    _validateIdTokenFromTokenRefreshToken(profile, id_token) {
        return Promise.all([
            ...
            this.settings.getEpochTime()
        ])
            .then(([issuer, now]) =>
                this._joseUtil.validateJwtAttributes(id_token, issuer, this._settings.client_id, this._settings.clockSkew, now))
            .then(payload => {
                if (!payload) {
    ...
    signinCallback(url) {
        return this.readSigninResponseState(url)
            .then(({state}) => {
    ...
    signoutCallback(url, keepOpen) {
        return this.readSignoutResponseState(url)
            .then(({state, response}) => {
                if (state) {
    ...
    querySessionStatus(args = {}) {
        ...
        return this._signinStart(args, this._iframeNavigator, {
            ...
            .then(navResponse => this.processSigninResponse(navResponse.url))
            .then(signinResponse => {
    ...
    _signin(args, navigator, navigatorParams = {}) {
        return this._signinStart(args, navigator, navigatorParams)
            .then(navResponse => this._signinEnd(navResponse.url, args));
    }
    ...
    _signinEnd(url, args = {}) {
        return this.processSigninResponse(url)
            .then(signinResponse => {
                ...
                return Promise.all([user, this.storeUser(user)]);
            })
            .then(([user]) => {
    ...
    signoutRedirect(args = {}) {
        ...
        return this._signoutStart(args, this._redirectNavigator, navParams)
            .then(() => {
        ...
        .then(handle => {
            ...
            return this._loadUser()
                .then(user => {
                    ...
                    const revokePromise = this._settings.revokeAccessTokenOnSignout ? this._revokeInternal(user) : Promise.resolve();
                ...
                .then(() => {
                    ...
                    return this.createSignoutRequest(args);
                })
                .then(signoutRequest => {
    ...
    revokeAccessToken() {
        return this._loadUser()
            .then(user => this._revokeInternal(user, true))
            .then(success => {
                if (success) {
    ...
    _revokeInternal(user, required) {
        if (user) {
            ...
            return this._revokeAccessTokenInternal(access_token, required)
                .then(atSuccess => Promise.all([
                    atSuccess,
                    this._revokeRefreshTokenInternal(refresh_token, required)
                ]))
                .then(([atSuccess, rtSuccess]) => {
    ...
    _loadUser() {
        return this._userStore.get(this._userStoreKey)
            .then(storageString => {
                if (storageString) {
